...
describe('Portfolio API 测试', () => {
  ...
  const sampleRiskRule = {
    id: 'rule-1',
    portfolioId: 'port-1',
    ruleType: 'MAX_SINGLE_POSITION',
    threshold: 0.3,
    isEnabled: true,
    createdAt: new Date(),
    updatedAt: new Date(),
  ...
  beforeEach(async () => {
    mockPortfolioService = {
      ...
      detail: jest.fn().mockResolvedValue({
        ...
        summary: {
          totalCost: 12500,
          totalMarketValue: 13200,
          totalUnrealizedPnl: 700,
          totalPnlPct: 0.056,
          cashBalance: 87500,
        },
      }),
      ...
      getPnlToday: jest.fn().mockResolvedValue({
        tradeDate: '2026-05-24',
        todayPnl: 320.5,
        todayPnlPct: 0.0243,
        byHolding: [{ tsCode: '000001.SZ', stockName: '平安银行', pctChg: 1.23, todayPnl: 150.6 }],
      }),
      getPnlHistory: jest
        .fn()
        .mockResolvedValue([{ date: new Date(), marketValue: 13200, costBasis: 12500, nav: 1.056 }]),
    }

    mockRiskService = {
      getIndustryDistribution: jest.fn().mockResolvedValue({
        tradeDate: '2026-05-24',
        industries: [{ industry: '银行', stockCount: 2, totalMarketValue: 26400, weight: 0.44 }],
      }),
      getPositionConcentration: jest.fn().mockResolvedValue({
        tradeDate: '2026-05-24',
        positions: [{ tsCode: '000001.SZ', stockName: '平安银行', marketValue: 13200, weight: 0.22 }],
        concentration: { hhi: 0.1234, top1Weight: 0.22, top3Weight: 0.55, top5Weight: 0.78 },
      }),
      getMarketCapDistribution: jest.fn().mockResolvedValue({
        tradeDate: '2026-05-24',
        buckets: [{ label: '大盘（> 1000亿）', count: 2, weight: 0.44 }],
      }),
      getBetaAnalysis: jest.fn().mockResolvedValue({
        tradeDate: '2026-05-24',
        portfolioBeta: 0.91,
        holdings: [{ tsCode: '000001.SZ', stockName: '平安银行', beta: 0.85, weight: 0.22 }],
      }),
    ...
    mockBridgeService = {
      applyBacktest: jest.fn().mockResolvedValue({
        portfolioId: 'port-1',
        portfolioName: '回测导入-价值策略',
        backtestRunId: 'bt-1',
        mode: 'REPLACE',
        snapshotDate: '2024-12-31',
        changes: [],
        summary: { added: 3, updated: 2, removed: 1, unchanged: 0, totalHoldings: 5 },
      }),
    }

    mockRebalanceService = {
      rebalancePlan: jest.fn().mockResolvedValue({
        portfolioId: 'port-1',
        portfolioName: '科技成长组合',
        refDate: '2026-05-24',
        totalValue: 500000,
        items: [
          {
            ...
            currentShares: 1000,
            currentPrice: 13.2,
            currentMarketValue: 13200,
            currentWeight: 0.22,
            targetWeight: 0.15,
            targetShares: 600,
            targetMarketValue: 7500,
            action: 'SELL',
            deltaShares: -400,
            deltaAmount: 5000,
            estimatedTradingCost: 6.25,
          },
        ],
        summary: {
          totalBuyAmount: 50000,
          totalSellProceeds: 30000,
          totalTradingCost: 87.5,
          buyCount: 3,
          sellCount: 2,
          adjustCount: 1,
          holdCount: 2,
  ...
  describe('组合 CRUD', () => {
    it('P-BIZ-001: 创建组合', async () => {
      ...
      expect(mockPortfolioService.create).toHaveBeenCalledWith(
        1,
        expect.objectContaining({ name: '科技成长组合', initialCash: 100000 }),
      )
    })

    it('P-BIZ-002: 获取组合列表', async () => {
      const res = await req.post('/portfolio/list').send({}).expect(201)
      expect(res.body.data).toHaveLength(1)
      expect(res.body.data[0].holdingCount).toBe(1)
    })

    it('P-BIZ-003: 获取组合详情', async () => {
      const res = await req.post('/portfolio/detail').send({ portfolioId: 'port-1' }).expect(201)
    ...
    it('P-BIZ-004: 更新组合', async () => {
      await req.post('/portfolio/update').send({ id: 'port-1', name: '更新后组合' }).expect(201)
      expect(mockPortfolioService.update).toHaveBeenCalledWith(
        expect.objectContaining({ id: 'port-1', name: '更新后组合' }),
        1,
      )
    })

    it('P-BIZ-005: 删除组合', async () => {
      const res = await req.post('/portfolio/delete').send({ portfolioId: 'port-1' }).expect(201)
      expect(res.body.data.success).toBe(true)
    })

    it('P-EDGE-001: 组合名称 100 字符', async () => {
      const name = 'a'.repeat(100)
      await req.post('/portfolio/create').send({ name, initialCash: 100000 }).expect(201)
    })

    it('P-EDGE-002: 组合名称 101 字符应 400', async () => {
      const name = 'a'.repeat(101)
      await req.post('/portfolio/create').send({ name, initialCash: 100000 }).expect(400)
    })

    it('P-EDGE-003: 初始资金=0', async () => {
      await req.post('/portfolio/create').send({ name: '零资金组合', initialCash: 0 }).expect(201)
    })

    it('P-EDGE-004: 初始资金负数应 400', async () => {
      await req.post('/portfolio/create').send({ name: '负资金组合', initialCash: -1 }).expect(400)
    })

    it('P-ERR-001: 创建缺 name 应 400', async () => {
      await req.post('/portfolio/create').send({ initialCash: 100000 }).expect(400)
    })

    it('P-ERR-002: 创建缺 initialCash 应 400', async () => {
      await req.post('/portfolio/create').send({ name: '缺资金' }).expect(400)
    })

    it('P-ERR-003: detail 不存在的组合', async () => {
      mockPortfolioService.detail.mockResolvedValue(null)
      const res = await req.post('/portfolio/detail').send({ portfolioId: '999999' }).expect(201)
      expect(res.body.data).toBeNull()
    })

    it('P-ERR-004: update 不存在的组合', async () => {
      mockPortfolioService.update.mockRejectedValue(new Error('组合不存在'))
      await req.post('/portfolio/update').send({ id: '999999', name: '不存在' }).expect(500)
    })

    it('P-ERR-005: delete 不存在的组合', async () => {
      mockPortfolioService.delete.mockRejectedValue(new Error('组合不存在'))
      await req.post('/portfolio/delete').send({ portfolioId: '999999' }).expect(500)
    })
  })
  ...
  describe('持仓管理', () => {
    ...
    it('P-BIZ-007: 更新持仓', async () => {
      await req
        ...
        .expect(201)
      expect(mockPortfolioService.updateHolding).toHaveBeenCalledWith(
        expect.objectContaining({ holdingId: 'hold-1', quantity: 2000 }),
        1,
      )
    })
  ...
  describe('盈亏分析', () => {
    it('P-BIZ-010: 当日盈亏', async () => {
      const res = await req.post('/portfolio/pnl/today').send({ portfolioId: 'port-1' }).expect(201)
    ...
    it('P-ERR-009: pnl/history 缺 endDate 应 400', async () => {
      await req.post('/portfolio/pnl/history').send({ portfolioId: 'port-1', startDate: '20260101' }).expect(400)
    })
  })
  ...
  describe('风控规则', () => {
    ...
    it('P-BIZ-018: 查询规则列表', async () => {
      const res = await req.post('/portfolio/rule/list').send({ portfolioId: 'port-1' }).expect(201)
      expect(res.body.data).toHaveLength(1)
      expect(res.body.data[0].ruleType).toBe('MAX_SINGLE_POSITION')
    })

    it('P-BIZ-019: 更新规则', async () => {
      await req.post('/portfolio/rule/update').send({ ruleId: 'rule-1', threshold: 0.4, isEnabled: true }).expect(201)
      expect(mockRiskCheckService.updateRule).toHaveBeenCalledWith(
        expect.objectContaining({ ruleId: 'rule-1', threshold: 0.4 }),
        1,
      )
    })

    it('P-BIZ-020: 删除规则', async () => {
      const res = await req.post('/portfolio/rule/delete').send({ ruleId: 'rule-1' }).expect(201)
      expect(res.body.data.success).toBe(true)
    })
    ...
    it('P-ERR-012: upsert 缺 portfolioId 应 400', async () => {
      await req.post('/portfolio/rule/upsert').send({ ruleType: 'MAX_SINGLE_POSITION', threshold: 0.3 }).expect(400)
    })
  })
  ...
  describe('风险检测', () => {
    it('P-BIZ-021: 执行风控检测', async () => {
      const res = await req.post('/portfolio/risk/check').send({ portfolioId: 'port-1' }).expect(201)
    ...
    it('P-BIZ-022: 查询违规记录', async () => {
      const res = await req.post('/portfolio/risk/violations').send({ portfolioId: 'port-1' }).expect(201)
      expect(res.body.data).toHaveLength(1)
  ...
  describe('调仓清单', () => {
    ...
    it('P-BIZ-027: 指定 totalValue', async () => {
      await req
        ...
        .expect(201)
      expect(mockRebalanceService.rebalancePlan).toHaveBeenCalledWith(
        expect.objectContaining({ totalValue: 500000 }),
        1,
      )
    })
    ...
    it('P-BIZ-028: 未指定持仓=SELL', async () => {
      await req
        ...
        .send({
          portfolioId: 'port-1',
          targets: [{ tsCode: '000001.SZ', targetWeight: 0.15 }],
          omitUnspecified: 'SELL',
        })
        .expect(201)
    })
    ...
    it('P-ERR-016: 缺 targets 应 400', async () => {
      await req.post('/portfolio/rebalance-plan').send({ portfolioId: 'port-1' }).expect(400)
    })
  })
  ...
  describe('绩效跟踪', () => {
    it('P-BIZ-030: 绩效查询', async () => {
      const res = await req.post('/portfolio/performance').send({ portfolioId: 'port-1' }).expect(201)
      expect(res.body.data.metrics.totalReturn).toBe(0.15)
      expect(res.body.data.dailySeries).toHaveLength(1)
    })

    it('P-BIZ-031: 指定基准', async () => {
      await req.post('/portfolio/performance').send({ portfolioId: 'port-1', benchmarkTsCode: '000001.SH' }).expect(201)
      expect(mockPerformanceService.getPerformance).toHaveBeenCalledWith(
        expect.objectContaining({ benchmarkTsCode: '000001.SH' }),
        1,
      )
    })
  ...
  describe('交易日志', () => {
    it('P-BIZ-034: 查询交易日志', async () => {
      const res = await req.post('/portfolio/trade-log').send({ portfolioId: 'port-1' }).expect(201)
      expect(res.body.data.items).toHaveLength(1)
    ...
    it('P-BIZ-035: 按日期过滤', async () => {
      await req
        ...
        .send({ portfolioId: 'port-1', startDate: '20260101', endDate: '20260524' })
        .expect(201)
      expect(mockTradeLogService.query).toHaveBeenCalledWith(
        expect.objectContaining({ startDate: '20260101', endDate: '20260524' }),
        1,
      )
    })

    it('P-BIZ-036: 按股票过滤', async () => {
      await req.post('/portfolio/trade-log').send({ portfolioId: 'port-1', tsCode: '000001.SZ' }).expect(201)
    ...
    it('P-BIZ-037: 交易日志汇总', async () => {
      const res = await req.post('/portfolio/trade-log/summary').send({ portfolioId: 'port-1' }).expect(201)
      expect(res.body.data.totalBuyAmount).toBe(50000)
      expect(res.body.data.tradeCount).toBe(10)
    })

    it('P-EDGE-021: 分页 page=1', async () => {
      await req.post('/portfolio/trade-log').send({ portfolioId: 'port-1', page: 1 }).expect(201)
    })

    it('P-EDGE-022: 分页 page=0 应 400', async () => {
      await req.post('/portfolio/trade-log').send({ portfolioId: 'port-1', page: 0 }).expect(400)
    })

    it('P-ERR-020: 日期格式错误应 400', async () => {
      await req.post('/portfolio/trade-log').send({ portfolioId: 'port-1', startDate: 'abc' }).expect(400)
    })
  })
  ...
  describe('安全', () => {
    it('P-SEC-001: 无 Token 应 401', async () => {
      ...
      await request(unauthApp.getHttpServer()).post('/portfolio/list').expect(401)
