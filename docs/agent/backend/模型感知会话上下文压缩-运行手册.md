...
- `L`：Run 的 `AGENT_MAX_INPUT_TOKENS`。
- `U`：Run 已消耗的输入 token。
...
每个 Run 首次进入 `load_context` 时通过真实 `ModelRouterService` 解析模型，不从目录列表猜测运行路由。解析结果写入 Workflow State/Checkpoint：
...
`AGENT_MAX_INPUT_TOKENS` 仍保留。它是单 Run 的成本/资源上限，不是会话压缩阈值；实际上下文预算取模型能力与 Run 剩余预算的更小值。
