...
type BacktestEngineTestApi = {
  checkRebalanceDay(date: Date, tradingDays: Date[], idx: number, frequency: RebalanceFrequency): boolean
  getTodayBars(allBarsMap: Map<string, Map<string, DailyBar>>, todayStr: string): Map<string, DailyBar>
  buildHistoricalBars(
    allBarsMap: Map<string, Map<string, DailyBar>>,
    upToDateStr: string,
    members?: Set<string>,
  ): Map<string, DailyBar[]>
  computePositionValueWithAdjFactor(portfolio: PortfolioState, todayBars: Map<string, DailyBar>): number
  adjustCostPriceForSplits(
    portfolio: PortfolioState,
    todayBars: Map<string, DailyBar>,
    yesterdayBars: Map<string, DailyBar>,
  ): void
  buildPositionSnapshots(portfolio: PortfolioState, todayBars: Map<string, DailyBar>, date: Date): PositionSnapshot[]
}
...
describe('BacktestEngineService', () => {
  ...
  let engineTestApi: BacktestEngineTestApi

  beforeEach(() => {
    jest.clearAllMocks()
    service = createService()
    engineTestApi = service as unknown as BacktestEngineTestApi
  })
  ...
  describe('checkRebalanceDay()', () => {
    ...
    it('DAILY: 每天都返回 true', () => {
      ...
      for (let i = 0; i < tradingDays.length; i++) {
        expect(engineTestApi.checkRebalanceDay(tradingDays[i], tradingDays, i, 'DAILY')).toBe(true)
      }
    })
    ...
    describe('WEEKLY', () => {
      it('第一个交易日（idx=0）返回 true', () => {
        ...
        expect(engineTestApi.checkRebalanceDay(tradingDays[0], tradingDays, 0, 'WEEKLY')).toBe(true)
      })

      it('同一周内第二天返回 false', () => {
        // 2025-01-02 (Thu) -> 2025-01-03 (Fri), 同一周
        const tradingDays = dates(['2025-01-02', '2025-01-03'])
        expect(engineTestApi.checkRebalanceDay(tradingDays[1], tradingDays, 1, 'WEEKLY')).toBe(false)
      })

      it('跨周（下周一）返回 true', () => {
        // 2025-01-03 (Fri) -> 2025-01-06 (Mon)
        const tradingDays = dates(['2025-01-03', '2025-01-06'])
        expect(engineTestApi.checkRebalanceDay(tradingDays[1], tradingDays, 1, 'WEEKLY')).toBe(true)
      })
      ...
      it('跨年但同一 ISO 周（周二→周四）→ 正确返回 false（同一交易周）', () => {
        ...
        const result = engineTestApi.checkRebalanceDay(tradingDays[1], tradingDays, 1, 'WEEKLY')
    ...
    describe('MONTHLY', () => {
      it('第一个交易日（idx=0）返回 true', () => {
        ...
        expect(engineTestApi.checkRebalanceDay(tradingDays[0], tradingDays, 0, 'MONTHLY')).toBe(true)
      })

      it('同月内第二天返回 false', () => {
        const tradingDays = dates(['2025-01-02', '2025-01-03'])
        expect(engineTestApi.checkRebalanceDay(tradingDays[1], tradingDays, 1, 'MONTHLY')).toBe(false)
      })

      it('跨月时返回 true', () => {
        const tradingDays = dates(['2025-01-31', '2025-02-03'])
        expect(engineTestApi.checkRebalanceDay(tradingDays[1], tradingDays, 1, 'MONTHLY')).toBe(true)
      })
    })
    ...
    describe('QUARTERLY', () => {
      it('第一个交易日（idx=0）返回 true', () => {
        const tradingDays = dates(['2025-01-02'])
        expect(engineTestApi.checkRebalanceDay(tradingDays[0], tradingDays, 0, 'QUARTERLY')).toBe(true)
      })

      it('同月内第二天返回 false', () => {
        const tradingDays = dates(['2025-01-02', '2025-01-03'])
        expect(engineTestApi.checkRebalanceDay(tradingDays[1], tradingDays, 1, 'QUARTERLY')).toBe(false)
      })

      it('跨入季度首月（1月=0）返回 true', () => {
        const tradingDays = dates(['2024-12-31', '2025-01-02'])
        expect(engineTestApi.checkRebalanceDay(tradingDays[1], tradingDays, 1, 'QUARTERLY')).toBe(true)
      })
  ...
  describe('getTodayBars()', () => {
    it('多只股票时返回指定日期的 bar', () => {
      ...
      const result: Map<string, DailyBar> = engineTestApi.getTodayBars(allBarsMap, '2025-01-02')
    ...
    it('当日无 bar 时返回空 Map', () => {
      ...
      const result: Map<string, DailyBar> = engineTestApi.getTodayBars(allBarsMap, '2025-01-09')
      expect(result.size).toBe(0)
    })

    it('allBarsMap 为空时返回空 Map', () => {
      const result: Map<string, DailyBar> = engineTestApi.getTodayBars(new Map(), '2025-01-02')
      expect(result.size).toBe(0)
    })
  })
  ...
  describe('buildHistoricalBars()', () => {
    it('返回不超过指定日期的 bar，并按日期升序排序', () => {
      ...
      const result: Map<string, DailyBar[]> = engineTestApi.buildHistoricalBars(allBarsMap, '2025-01-02')
  ...
  describe('computePositionValueWithAdjFactor()', () => {
    it('正确累加多个持仓的市值', () => {
      ...
      const value: number = engineTestApi.computePositionValueWithAdjFactor(portfolio, todayBars)
    ...
    it('bar 缺失时使用 costPrice 兜底', () => {
      ...
      const value: number = engineTestApi.computePositionValueWithAdjFactor(portfolio, new Map())
  ...
  describe('adjustCostPriceForSplits()', () => {
    it('adjFactor 变化时调整 costPrice 和 quantity', () => {
      ...
      const todayBars = new Map<string, DailyBar>([['000001.SZ', buildBar({ tsCode: '000001.SZ', adjFactor: 2.0 })]])
      const yesterdayBars = new Map<string, DailyBar>([
        ['000001.SZ', buildBar({ tsCode: '000001.SZ', adjFactor: 1.0 })],
      ])

      engineTestApi.adjustCostPriceForSplits(portfolio, todayBars, yesterdayBars)
    ...
    it('adjFactor 相同时不调整', () => {
      ...
      const todayBars = new Map<string, DailyBar>([['000001.SZ', buildBar({ tsCode: '000001.SZ', adjFactor: 1.0 })]])
      const yesterdayBars = new Map<string, DailyBar>([
        ['000001.SZ', buildBar({ tsCode: '000001.SZ', adjFactor: 1.0 })],
      ])
    ...
    it('adjFactor 为 null 时不调整', () => {
      ...
      const todayBars = new Map<string, DailyBar>([['000001.SZ', buildBar({ tsCode: '000001.SZ', adjFactor: null })]])
      const yesterdayBars = new Map<string, DailyBar>([
        ['000001.SZ', buildBar({ tsCode: '000001.SZ', adjFactor: 1.0 })],
      ])
  ...
  describe('buildPositionSnapshots()', () => {
    it('返回每个持仓的快照，包含 weight、unrealizedPnl、holdingDays', () => {
      ...
      const todayBars = new Map<string, DailyBar>([['000001.SZ', buildBar({ tsCode: '000001.SZ', close: 10 })]])

      const snapshots = engineTestApi.buildPositionSnapshots(portfolio, todayBars, today)
    ...
    it('bar 缺失时 closePrice/marketValue/unrealizedPnl 均为 null', () => {
      ...
      const snapshots = engineTestApi.buildPositionSnapshots(portfolio, new Map(), new Date('2025-01-11'))
    ...
    it('空持仓时返回空数组', () => {
      const portfolio = buildPortfolio({ positions: new Map() })
      const snapshots = engineTestApi.buildPositionSnapshots(portfolio, new Map(), new Date())
  ...
  describe('[EDGE] adjustCostPriceForSplits() 精度与边界', () => {
    it('[EDGE] 多次连续送转（10送10 三次）累积不产生错误', () => {
      ...
      // 第一次 10送10: ratio=2
      engineTestApi.adjustCostPriceForSplits(
        portfolio,
  ...
  describe('runBacktest()', () => {
    ...
    it('[EDGE] benchmark 中间缺日时沿用上一日 benchmarkNav，不产生虚假回撤', async () => {
      ...
      const strategyRegistry = {
        getStrategy: jest.fn(() => ({ generateSignal: jest.fn(async () => ({ targets: [] })) })),
      }
