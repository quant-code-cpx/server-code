...
| --- | -------------------------- | ---------------------------- | ---------- | --------- |
| 1   | 核心指数行情               | `index_daily`                | 2000       | ✅ 已完成 |
| 2   | 大盘资金流向               | `moneyflow_mkt_dc`           | 2000       | ✅ 已同步 |
| 3   | 行业/概念/地域板块流向     | `moneyflow_ind_dc`           | 2000       | ✅ 已同步 |
| 4   | 市场情绪（涨跌家数统计）   | 聚合 `daily` + `daily_basic` | 无需新接口 | ✅ 已完成 |
| 5   | 市场整体估值（PE/PB 分位） | 聚合 `daily_basic`           | 无需新接口 | ✅ 已完成 |
| 6   | 涨跌停板数据               | `limit_list_d`               | 2000       | ❌        |
| 7   | 北向/南向资金（沪深港通）  | `moneyflow_hsgt`             | 2000       | ✅ 已完成 |
...
- [x] 新建 Prisma 模型 `IndexDaily`（`prisma/tushare_index_daily.prisma`）
- [x] 在 `TushareApiName` 枚举中加入 `INDEX_DAILY = 'index_daily'`
- [x] 在 `MarketApiService` 中新增 `getCoreIndexDailyByTradeDate()`（逐只串行拉取6支核心指数）
- [x] 在 `market-sync.service.ts` 中新增 `syncIndexDaily()`（只同步 CORE_INDEX_CODES 中的 6 支指数）
- [x] 在 `MarketService` 中新增 `getIndexQuote()` 查询方法
- [x] 在 `MarketController` 中新增 `POST /market/index-quote` 接口
...
- [x] 在 `MarketService` 中新增 `getMarketSentiment()` 方法
  - 按 `pct_chg` 范围分桶统计：`≤-5%`、`-5%~0%`、`≈0%`、`0%~5%`、`≥5%`
- [x] 在 `MarketController` 中新增 `POST /market/sentiment` 接口
...
- [x] 在 `MarketService` 中新增 `getMarketValuation()` 方法
  - 使用 PostgreSQL `PERCENTILE_CONT(0.5)` 计算当日 PE_TTM / PB 中位数
  - 取过去 1/3/5 年每日中位数序列，计算当日在历史分位（百分位数）
- [x] 在 `MarketController` 中新增 `POST /market/valuation` 接口
...
- [x] 新建 Prisma 模型 `MoneyflowHsgt`（`prisma/tushare_moneyflow_hsgt.prisma`）
- [x] 在 `TushareApiName` 枚举中加入 `MONEYFLOW_HSGT = 'moneyflow_hsgt'`
- [x] 在 `MoneyflowApiService` 中新增 `getMoneyflowHsgtByDateRange()` 方法
- [x] 在 `MoneyflowSyncService` 中新增 `syncMoneyflowHsgt()` 同步逻辑（按日期区间拉取）
- [x] 在 `MarketService` 中新增 `getHsgtFlow()` 查询方法（返回最近 20 日趋势）
- [x] 在 `MarketController` 中新增 `POST /market/hsgt-flow` 接口
