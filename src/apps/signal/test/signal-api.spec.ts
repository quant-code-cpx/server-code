...
describe('Signal API 测试', () => {
      ...
      compareHistories: jest.fn().mockResolvedValue([
        {
          strategyId: 'strat-1',
          strategyName: '策略A',
          summary: { total: 10, buyCount: 4, sellCount: 3, holdCount: 3, avgConfidence: 0.7 },
        },
        {
          strategyId: 'strat-2',
          strategyName: '策略B',
          summary: { total: 8, buyCount: 3, sellCount: 2, holdCount: 3, avgConfidence: 0.6 },
        },
      ]),
    }
  ...
  describe('策略激活', () => {
    it('SG-BIZ-001: 激活策略', async () => {
      const res = await req.post('/signal/strategies/activate').send({ strategyId: 'strat-1' }).expect(201)
    ...
    it('SG-BIZ-002: 停用策略', async () => {
      const res = await req.post('/signal/strategies/deactivate').send({ strategyId: 'strat-1' }).expect(201)
      expect(res.body.data.isActive).toBe(false)
    })

    it('SG-BIZ-003: 查询激活列表', async () => {
      const res = await req.post('/signal/strategies/list').send({}).expect(201)
    ...
    it('SG-ERR-001: activate 缺 strategyId 应 400', async () => {
      await req.post('/signal/strategies/activate').send({}).expect(400)
    })

    it('SG-ERR-002: deactivate 缺 strategyId 应 400', async () => {
      await req.post('/signal/strategies/deactivate').send({}).expect(400)
    })
  })
  ...
  describe('信号查询', () => {
    it('SG-BIZ-004: 查询最新信号', async () => {
      const res = await req.post('/signal/latest').send({ strategyId: 'strat-1' }).expect(201)
      expect(Array.isArray(res.body.data)).toBe(true)
    ...
    it('SG-BIZ-005: 查询信号历史', async () => {
      const res = await req.post('/signal/history').send({ strategyId: 'strat-1' }).expect(201)
    ...
    it('SG-EDGE-001: tradeDate 格式正确', async () => {
      await req.post('/signal/latest').send({ tradeDate: '20260523' }).expect(201)
    })

    it('SG-EDGE-002: tradeDate 格式错误应 400', async () => {
      await req.post('/signal/latest').send({ tradeDate: '2026-05-23' }).expect(400)
    })

    it('SG-EDGE-003: alertThreshold=0（最小）应 201', async () => {
      await req.post('/signal/strategies/activate').send({ strategyId: 'strat-1', alertThreshold: 0 }).expect(201)
    })

    it('SG-EDGE-004: alertThreshold=1（最大）应 201', async () => {
      await req.post('/signal/strategies/activate').send({ strategyId: 'strat-1', alertThreshold: 1 }).expect(201)
    })

    it('SG-EDGE-005: alertThreshold=-0.1 应 400', async () => {
      await req.post('/signal/strategies/activate').send({ strategyId: 'strat-1', alertThreshold: -0.1 }).expect(400)
    })

    it('SG-EDGE-006: alertThreshold=1.1 应 400', async () => {
      await req.post('/signal/strategies/activate').send({ strategyId: 'strat-1', alertThreshold: 1.1 }).expect(400)
    })

    it('SG-EDGE-007: forwardWindow=1（最小）应 201', async () => {
      await req.post('/signal/history').send({ strategyId: 'strat-1', forwardWindow: 1 }).expect(201)
    })

    it('SG-EDGE-008: forwardWindow=60（最大）应 201', async () => {
      await req.post('/signal/history').send({ strategyId: 'strat-1', forwardWindow: 60 }).expect(201)
    })

    it('SG-EDGE-009: forwardWindow=0 应 400', async () => {
      await req.post('/signal/history').send({ strategyId: 'strat-1', forwardWindow: 0 }).expect(400)
    })

    it('SG-EDGE-010: forwardWindow=61 应 400', async () => {
      await req.post('/signal/history').send({ strategyId: 'strat-1', forwardWindow: 61 }).expect(400)
    })

    it('SG-ERR-003: history 缺 strategyId 应 400', async () => {
      await req.post('/signal/history').send({}).expect(400)
    })
  ...
  describe('安全', () => {
    it('SG-SEC-001: 无 Token 应 401', async () => {
      ...
      await request(unauthApp.getHttpServer()).post('/signal/latest').expect(401)
