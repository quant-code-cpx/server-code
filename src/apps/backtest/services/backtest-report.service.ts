...
export class BacktestReportService {
  ...
  async saveReport(runId: string, result: BacktestResult): Promise<void> {
    ...
    await this.prisma.$transaction(async (tx) => {
      await tx.backtestDailyNav.deleteMany({ where: { runId } })
      await tx.backtestTrade.deleteMany({ where: { runId } })
      await tx.backtestPositionSnapshot.deleteMany({ where: { runId } })
      await tx.backtestRebalanceLog.deleteMany({ where: { runId } })

      // Save daily navs
      if (navRecords.length > 0) {
        await tx.backtestDailyNav.createMany({
      ...
      if (positions.length > 0) {
        await tx.backtestPositionSnapshot.createMany({
      ...
      if (rebalanceLogs.length > 0) {
        await tx.backtestRebalanceLog.createMany({
