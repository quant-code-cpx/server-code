...
上下文顺序固定：系统/安全规则 → workflow/prompt 固定版本 → 页面上下文与结构化会话状态 → 版本化滚动摘要 → 摘要之后的最近原始消息 → 本轮已完成 Tool 事实包 → 显式用户记忆。系统规则与当前问题不可裁剪；隐藏推理不持久化或暴露；只保存结构化计划摘要、决策原因类别和可审计输入输出。`pageContext` 仅为提示线索，资源权限仍由 Tool/owner 校验。
