...
export class StrategyDraftService {
  ...
  async createDraft(userId: number, dto: CreateStrategyDraftDto) {
    ...
    try {
      return await this.prisma.strategyDraft.create({
        data: { userId, name: dto.name, config: dto.config as Parameters<typeof this.prisma.strategyDraft.create>[0]['data']['config'] },
      })
    } catch (e: unknown) {
    ...
    try {
      return await this.prisma.strategyDraft.update({
        ...
        data: {
          ...
          ...(dto.config !== undefined && { config: dto.config as Parameters<typeof this.prisma.strategyDraft.update>[0]['data']['config'] }),
        },
      })
    } catch (e: unknown) {
      if ((e as Prisma.PrismaClientKnownRequestError).code === 'P2002') throw new ConflictException('同名草稿已存在')
  ...
  async submitDraft(userId: number, draftId: number, dto: SubmitDraftDto) {
    ...
    return this.backtestRunService.createRun(backtestDto as Parameters<typeof this.backtestRunService.createRun>[0], userId)
  }
}
