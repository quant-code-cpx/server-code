...
import type { MonteCarloResultDto } from '../dto/monte-carlo.dto'
...
type MonteCarloTestApi = {
  makeRng(seed?: number): () => number
  compute(
    navRows: Array<{ nav: Prisma.Decimal | number | null; dailyReturn: number | null }>,
    options: { numSimulations: number; confidenceLevels: number[]; seed?: number },
  ): MonteCarloResultDto
}
...
describe('BacktestMonteCarloService', () => {
  ...
  let monteCarloTestApi: MonteCarloTestApi

  beforeEach(() => {
    jest.clearAllMocks()
    service = createService()
    monteCarloTestApi = service as unknown as MonteCarloTestApi
  })
  ...
  describe('makeRng()', () => {
    it('固定种子产生确定性序列', () => {
      const rng1 = monteCarloTestApi.makeRng(42)
      const rng2 = monteCarloTestApi.makeRng(42)
    ...
    it('所有值在 [0, 1) 范围内', () => {
      const rng = monteCarloTestApi.makeRng(123)
      for (let i = 0; i < 100; i++) {
        const v = rng()
    ...
    it('不同种子产生不同序列', () => {
      const rng1 = monteCarloTestApi.makeRng(1)
      const rng2 = monteCarloTestApi.makeRng(2)
  ...
  describe('compute()', () => {
    ...
    it('finalNavDistribution.mean 在合理范围内（0.5 ~ 2.0）', () => {
      ...
      const result = monteCarloTestApi.compute(navRows, defaultOptions)
    ...
    it('固定种子产生完全相同的结果（可重现性）', () => {
      ...
      const result1 = monteCarloTestApi.compute(navRows, { ...defaultOptions, seed: 99 })
      const result2 = monteCarloTestApi.compute(navRows, { ...defaultOptions, seed: 99 })
    ...
    it('单日收益也能产生有效结果', () => {
      ...
      const result = monteCarloTestApi.compute(navRows, { ...defaultOptions, numSimulations: 100 })
    ...
    it('numSimulations 正确传入并反映在输出中', () => {
      ...
      const result = monteCarloTestApi.compute(navRows, { ...defaultOptions, numSimulations: 200 })
  ...
  describe('runMonteCarloSimulation()', () => {
    it('run 不存在时抛出 NotFoundException', async () => {
      ...
      await expect(svc.runMonteCarloSimulation('nonexistent-id', {}, 1)).rejects.toThrow(NotFoundException)
    })
    ...
    it('非本人 run 时抛出 NotFoundException，且不读取 NAV', async () => {
      ...
      await expect(svc.runMonteCarloSimulation('run-1', {}, 1)).rejects.toThrow(NotFoundException)
    ...
    it('正常数据时返回包含 finalNavDistribution 的结果', async () => {
      ...
      const result = await svc.runMonteCarloSimulation('run-1', { numSimulations: 100, seed: 42 }, 1)
