...
describe('Strategy API 测试', () => {
  ...
  describe('策略 CRUD', () => {
    ...
    it('S-BIZ-002: 查询策略列表', async () => {
      const res = await req.post('/strategies/list').send({}).expect(201)
      expect(res.body.data.strategies).toHaveLength(1)
      expect(res.body.data.total).toBe(1)
    })

    it('S-BIZ-003: 查询策略详情', async () => {
      const res = await req.post('/strategies/detail').send({ id: 'strat-1' }).expect(201)
      expect(res.body.data.id).toBe('strat-1')
      expect(res.body.data.strategyConfig).toHaveProperty('shortPeriod')
    })

    it('S-BIZ-004: 更新策略', async () => {
      await req.post('/strategies/update').send({ id: 'strat-1', name: '更新后策略' }).expect(201)
      expect(mockStrategyService.update).toHaveBeenCalledWith(
        1,
        expect.objectContaining({ id: 'strat-1', name: '更新后策略' }),
      )
    })

    it('S-BIZ-005: 删除策略', async () => {
      await req.post('/strategies/delete').send({ id: 'strat-1' }).expect(201)
      expect(mockStrategyService.delete).toHaveBeenCalledWith(1, 'strat-1', undefined)
    })

    it('S-BIZ-006: 克隆策略', async () => {
      const res = await req.post('/strategies/clone').send({ id: 'strat-1', name: '克隆策略' }).expect(201)
    ...
    it('S-ERR-001: 创建缺 name 应 400', async () => {
      await req.post('/strategies/create').send({ strategyType: 'MA_CROSS_SINGLE', strategyConfig: {} }).expect(400)
    })

    it('S-ERR-002: 创建缺 strategyType 应 400', async () => {
      await req.post('/strategies/create').send({ name: 'test', strategyConfig: {} }).expect(400)
    })

    it('S-ERR-003: 创建缺 strategyConfig 应 400', async () => {
      await req.post('/strategies/create').send({ name: 'test', strategyType: 'MA_CROSS_SINGLE' }).expect(400)
    })
  ...
  describe('Schema', () => {
    it('S-BIZ-008: 获取 schemas', async () => {
      const res = await req.post('/strategies/schemas').send({}).expect(201)
  ...
  describe('版本管理', () => {
    it('S-BIZ-009: 查询版本列表', async () => {
      const res = await req.post('/strategies/versions').send({ strategyId: 'strat-1' }).expect(201)
    ...
    it('S-ERR-013: versions 缺 strategyId 应 400', async () => {
      await req.post('/strategies/versions').send({}).expect(400)
    })

    it('S-ERR-014: compare 缺 versionA 应 400', async () => {
      await req.post('/strategies/compare-versions').send({ strategyId: 'strat-1', versionB: 2 }).expect(400)
    })
  })
  ...
  describe('统计', () => {
    it('S-BIZ-011: 策略汇总', async () => {
      const res = await req.post('/strategies/summary').send({}).expect(201)
      expect(res.body.data.total).toBe(3)
      expect(res.body.data.byType).toHaveProperty('MA_CROSS_SINGLE')
    })

    it('S-BIZ-012: 查询业绩', async () => {
      const res = await req.post('/strategies/performance').send({}).expect(201)
      expect(res.body.data.items).toHaveLength(1)
      expect(res.body.data.items[0].totalReturn).toBe(0.15)
    })

    it('S-BIZ-013: 按策略 ID 过滤业绩', async () => {
      await req.post('/strategies/performance').send({ strategyId: 'strat-1' }).expect(201)
      expect(mockStrategyService.performance).toHaveBeenCalledWith(
        1,
  ...
  describe('安全', () => {
    it('S-SEC-001: 无 Token 应 401', async () => {
      ...
      await request(unauthApp.getHttpServer()).post('/strategies/list').expect(401)
