...
describe('Backtest API 测试', () => {
  ...
  beforeEach(async () => {
    mockRunService = {
      ...
      listRuns: jest.fn().mockResolvedValue({
        items: [{ id: 'run-1', name: '测试回测', status: 'COMPLETED' }],
        total: 1,
        page: 1,
        pageSize: 20,
      }),
      getRunDetail: jest
        .fn()
        .mockResolvedValue({ id: 'run-1', name: '测试回测', status: 'COMPLETED', strategyType: 'MA_CROSS_SINGLE' }),
      getEquity: jest.fn().mockResolvedValue([{ tradeDate: '20260524', nav: 1.15 }]),
      getTrades: jest
        .fn()
        .mockResolvedValue({ items: [{ tsCode: '000001.SZ', action: 'BUY', quantity: 100 }], total: 1 }),
    ...
    mockStrategyRegistry = {
      getTemplates: jest
        .fn()
        .mockResolvedValue([{ type: 'MA_CROSS_SINGLE', name: '均线交叉', description: '双均线策略' }]),
    }

    mockWalkForwardService = {
      createWalkForwardRun: jest.fn().mockResolvedValue({ wfRunId: 'wf-1', jobId: 'job-1', status: 'QUEUED' }),
      listWalkForwardRuns: jest
        .fn()
        .mockResolvedValue({ items: [{ wfRunId: 'wf-1', status: 'COMPLETED' }], total: 1, page: 1, pageSize: 20 }),
      getWalkForwardRunDetail: jest.fn().mockResolvedValue({ wfRunId: 'wf-1', status: 'COMPLETED', windows: [] }),
      getWalkForwardEquity: jest
        .fn()
        .mockResolvedValue({ points: [{ tradeDate: '20260524', nav: 1.1, windowIndex: 0 }] }),
    ...
    mockParamSensitivityService = {
      create: jest
        .fn()
        .mockResolvedValue({ sweepId: 'sweep-1', totalCombinations: 16, status: 'PENDING', metric: 'sharpeRatio' }),
  ...
  describe('回测 CRUD', () => {
    ...
    it('BT-BIZ-001: 创建回测', async () => {
      const res = await req.post('/backtests/runs').send(validRunDto).expect(201)
      expect(res.body.data.runId).toBe('run-1')
      expect(res.body.data.status).toBe('QUEUED')
    })

    it('BT-BIZ-002: 查询回测列表', async () => {
      const res = await req.post('/backtests/runs/list').send({}).expect(201)
      expect(res.body.data.items).toHaveLength(1)
      expect(res.body.data.total).toBe(1)
    })

    it('BT-BIZ-003: 获取回测详情', async () => {
      const res = await req.post('/backtests/runs/detail').send({ runId: 'run-1' }).expect(201)
      expect(res.body.data.id).toBe('run-1')
    })

    it('BT-BIZ-004: 取消回测', async () => {
      const res = await req.post('/backtests/runs/cancel').send({ runId: 'run-1' }).expect(201)
      expect(res.body.data.status).toBe('CANCELLED')
    })

    it('BT-BIZ-005: 重命名回测', async () => {
      await req.post('/backtests/runs/rename').send({ runId: 'run-1', name: '新名称' }).expect(201)
    })

    it('BT-BIZ-006: 归档回测', async () => {
      await req.post('/backtests/runs/archive').send({ runId: 'run-1', archived: true }).expect(201)
    })

    it('BT-BIZ-007: 删除回测', async () => {
      await req.post('/backtests/runs/delete').send({ runId: 'run-1' }).expect(201)
    })

    it('BT-BIZ-008: 标星回测', async () => {
      await req.post('/backtests/runs/star').send({ runId: 'run-1', starred: true }).expect(201)
    })

    it('BT-BIZ-009: 重试回测', async () => {
      await req.post('/backtests/runs/retry').send({ runId: 'run-1' }).expect(201)
    })
    ...
    it('BT-EDGE-003: 日期格式正确', async () => {
      await req.post('/backtests/runs').send(validRunDto).expect(201)
    })
    ...
    it('BT-ERR-003: 缺 startDate 应 400', async () => {
      ...
      void startDate
      await req.post('/backtests/runs').send(rest).expect(400)
    })
  })
  ...
  describe('回测数据', () => {
    it('BT-BIZ-010: 获取净值曲线', async () => {
      const res = await req.post('/backtests/runs/equity').send({ runId: 'run-1' }).expect(201)
      expect(res.body.data).toHaveLength(1)
      expect(res.body.data[0].nav).toBe(1.15)
    })

    it('BT-BIZ-011: 查询交易明细', async () => {
      const res = await req.post('/backtests/runs/trades').send({ runId: 'run-1' }).expect(201)
      expect(res.body.data.items).toHaveLength(1)
    })

    it('BT-BIZ-012: 查询持仓快照', async () => {
      const res = await req.post('/backtests/runs/positions').send({ runId: 'run-1' }).expect(201)
      expect(res.body.data).toHaveLength(1)
    })

    it('BT-BIZ-013: 查询调仓日志', async () => {
      const res = await req.post('/backtests/runs/rebalance-logs').send({ runId: 'run-1' }).expect(201)
      expect(res.body.data).toHaveLength(1)
    })
  })
  ...
  describe('高级分析', () => {
    ...
    it('BT-BIZ-015: Brinson 归因', async () => {
      const res = await req.post('/backtests/runs/attribution').send({ runId: 'run-1' }).expect(201)
      expect(res.body.data.runId).toBe('run-1')
    })

    it('BT-BIZ-016: 成本敏感性分析', async () => {
      const res = await req.post('/backtests/runs/cost-sensitivity').send({ runId: 'run-1' }).expect(201)
      expect(res.body.data.runId).toBe('run-1')
    })
    ...
    it('BT-BIZ-018: 查询参数扫描结果', async () => {
      const res = await req.post('/backtests/runs/param-sensitivity/result').send({ sweepId: 'sweep-1' }).expect(201)
      expect(res.body.data.heatmap).toHaveLength(2)
    })

    it('BT-EDGE-007: monte-carlo numSimulations=100', async () => {
      await req.post('/backtests/runs/monte-carlo').send({ runId: 'run-1', numSimulations: 100 }).expect(201)
    })

    it('BT-EDGE-008: monte-carlo numSimulations=10000', async () => {
      await req.post('/backtests/runs/monte-carlo').send({ runId: 'run-1', numSimulations: 10000 }).expect(201)
    })

    it('BT-EDGE-009: monte-carlo numSimulations=99', async () => {
      // RunMonteCarloDto 使用 @IsOptional()，@Min(100) 在可选字段上的行为待澄清
      const res = await req.post('/backtests/runs/monte-carlo').send({ runId: 'run-1', numSimulations: 99 })
  ...
  describe('Walk-Forward', () => {
    ...
    it('BT-BIZ-019: 创建 Walk-Forward', async () => {
      const res = await req.post('/backtests/walk-forward/runs').send(validWfDto).expect(201)
      expect(res.body.data.wfRunId).toBe('wf-1')
    })

    it('BT-BIZ-020: Walk-Forward 列表', async () => {
      const res = await req.post('/backtests/walk-forward/runs/list').send({}).expect(201)
      expect(res.body.data.items).toHaveLength(1)
    })

    it('BT-BIZ-021: Walk-Forward 详情', async () => {
      const res = await req.post('/backtests/walk-forward/runs/detail').send({ wfRunId: 'wf-1' }).expect(201)
      expect(res.body.data.wfRunId).toBe('wf-1')
    })

    it('BT-BIZ-022: Walk-Forward 净值', async () => {
      const res = await req.post('/backtests/walk-forward/runs/equity').send({ wfRunId: 'wf-1' }).expect(201)
  ...
  describe('多策略对比', () => {
    ...
    it('BT-BIZ-023: 创建对比组', async () => {
      const res = await req.post('/backtests/comparisons').send(validComparisonDto).expect(201)
      expect(res.body.data.groupId).toBe('grp-1')
    })

    it('BT-BIZ-024: 对比组列表', async () => {
      const res = await req.post('/backtests/comparisons/list').send({}).expect(201)
      expect(res.body.data.items).toHaveLength(1)
    })

    it('BT-BIZ-025: 对比组详情', async () => {
      const res = await req.post('/backtests/comparisons/detail').send({ groupId: 'grp-1' }).expect(201)
      expect(res.body.data.groupId).toBe('grp-1')
    })

    it('BT-BIZ-026: 对比组净值', async () => {
      const res = await req.post('/backtests/comparisons/equity').send({ groupId: 'grp-1' }).expect(201)
      expect(res.body.data.series).toBeDefined()
    })

    it('BT-EDGE-014: strategies=2 个（最小）', async () => {
      await req.post('/backtests/comparisons').send(validComparisonDto).expect(201)
    })
  ...
  describe('统计', () => {
    it('BT-BIZ-027: 回测统计', async () => {
      const res = await req.post('/backtests/runs/stats').send({}).expect(201)
      expect(res.body.data.total).toBe(10)
    })

    it('BT-BIZ-028: 策略模板列表', async () => {
      const res = await req.post('/backtests/strategy-templates').send({}).expect(201)
      expect(res.body.data).toHaveLength(1)
    })
  })
  ...
  describe('安全', () => {
    it('BT-SEC-001: 无 Token 应 401', async () => {
      ...
      await request(unauthApp.getHttpServer()).post('/backtests/runs/list').expect(401)
