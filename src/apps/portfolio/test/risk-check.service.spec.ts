...
describe('RiskCheckService', () => {
  ...
  // ── formatDate 时区修复（B3 已修复）──────────────────────────────────────

  describe('[B3] formatDate() — 使用上海时区（已修复）', () => {
    ...
    it('formatDate 对 2025-03-01T16:00:00Z（上海 3月2日 00:00）返回 "20250302"', () => {
  ...
  // 修复后：使用 dayjs(latestDate).subtract(1, 'year') 替代 setFullYear
  // dayjs 处理 2024-02-29 → 自动 clamp 到 2023-02-28

  describe('[B7] checkMaxDrawdown — 闰年向前推 1 年使用 dayjs.subtract（已修复）', () => {
    it('JavaScript setFullYear 对 Feb29 的溢出（保留为参考验证）', async () => {
      // 保留：说明旧方法为何有 bug（溢出到 Mar1）
      const leapDay = new Date('2024-02-29')
      leapDay.setFullYear(2023)
      expect(leapDay.getMonth() + 1).toBe(3) // 溢出到 3月（JS 原生行为）
    })

    it('latestDate=2024-02-29 时 checkMaxDrawdown 正常走通（不因日期溢出崩溃）', async () => {
      const prisma = buildPrismaMock()
      const portfolioSvc = buildPortfolioServiceMock()
      portfolioSvc.getLatestTradeDate.mockResolvedValue(new Date('2024-02-29'))
      prisma.$queryRaw.mockResolvedValue([
        { trade_date: new Date('2023-02-28'), market_value: 1000, cost_basis: 1000 },
  ...
  // 修复后：peak = 1.0（投资起始净值），避免回撤被低估

  describe('[B8] checkMaxDrawdown — peak 从 1.0 初始化（已修复）', () => {
    it('navs=[0.95,0.8,0.85]：最大回撤为 20%（从初始 1.0 计算）', async () => {
      // 手算（修复后 peak=1.0）：
      //   navs[0]=0.95 → dd=(1.0-0.95)/1.0=0.05
      //   navs[1]=0.80 → peak=1.0 → dd=(1.0-0.80)/1.0=0.20 ← 最大
      //   navs[2]=0.85 → dd=(1.0-0.85)/1.0=0.15
      //   maxDrawdown=0.20
      const prisma = buildPrismaMock()
      const portfolioSvc = buildPortfolioServiceMock()
      ...
      const rule = buildRule({
        ruleType: PortfolioRiskRuleType.MAX_DRAWDOWN_STOP,
        threshold: 0.25, // 25%：maxDrawdown=20% < 25% → 不触发
      })
      const svc = createService(prisma, portfolioSvc)
    ...
    it('navs=[0.95,0.75,0.9]: maxDrawdown=25% > 24% 阈值 → 正确触发（修复后）', async () => {
      // 手算（修复后 peak=1.0）：
      //   navs[1]=0.75 → dd=(1.0-0.75)/1.0=0.25 > 0.24 → 触发
      // 旧实现（BUG）：peak=navs[0]=0.95 → dd=(0.95-0.75)/0.95≈0.2105 < 0.24 → 漏报
      const prisma = buildPrismaMock()
      const portfolioSvc = buildPortfolioServiceMock()
      portfolioSvc.getLatestTradeDate.mockResolvedValue(new Date('2025-03-01'))
      prisma.$queryRaw.mockResolvedValue([
        { trade_date: new Date('2025-01-01'), market_value: 950, cost_basis: 1000 },
        { trade_date: new Date('2025-02-01'), market_value: 750, cost_basis: 1000 },
        { trade_date: new Date('2025-03-01'), market_value: 900, cost_basis: 1000 },
      ])

      const rule = buildRule({
        ruleType: PortfolioRiskRuleType.MAX_DRAWDOWN_STOP,
        threshold: 0.24, // 24%：修复后 maxDrawdown=25% > 24% → 触发
      })
      const svc = createService(prisma, portfolioSvc)
      ...
      expect(result?.actualValue).toBeCloseTo(0.25, 5)
    })
  })
