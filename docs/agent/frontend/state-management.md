...
| 流恢复位置、活动信号、未校验草稿与去重窗口 | 服务端事件流 + 当前连接 | 仅内存；必要时用服务端状态恢复 |
...
模型流保持两个独立投影：`modelActivity` 只保存模型调用 ID、公开 phase 和累计字符数；`draftPreview` 只保存模型调用 ID、修复 attempt 与最多 8,000 字符的未校验 `markdown`。两者都不属于 `AgentMessageEntity.contentText`，`model.delta` 开始或 Run 终态时必须清空，避免草稿被误认为权威消息。
